    ...
    <div class="container content">
            <div class="col-md-12">
                @if ($task->status == 'In Queue')
                    <i class="fa fa-5x fa-clock-o pull-right text-success"></i>
                @else
                    <i class="fa fa-5x fa-check-circle pull-right text-success"></i>
                @endif
                <h1>Task #{{ $task->id }}</h1>
                        ...
                        @foreach ($task->details as $detail)
                            <tr>
                                ...
                                <td style="word-break:break-all">
                                    @if($detail->result!='')
                                        @for($i = 0; $i < strlen($detail->result); $i++)
                                        @if($detail->result[$i]==0)
                                            {{ $detail->sequence[$i] }}
                                        @else
                                            <b style="color: #f00">{{ $detail->sequence[$i] }}</b>
                                        @endif
                                    @endfor
                                    @else
                                        {{ $detail->sequence }}
